...
const Navbar = ({ setShowLogin }) => {
  ...
  return (
    <nav className="w-full  shadow-md fixed z-50 bg-black/3 backdrop-blur-xs">
      <div className="flex justify-between items-center px-4 lg:px-20 py-2">

        {/* Logo */}
        <Link to={'/'} className="flex items-center">
          <img src={FRassets.logo} alt="Logo" className="h-5 lg:h-7 my-3" />
        </Link>
        ...
        <div className="flex items-center gap-5 lg:gap-10">
          ...
          {/* Profile / Login */}
          {!token ? (
            <button onClick={() => setShowLogin(true)} className="hidden lg:block text-black border-2 border-gray-400 px-4 py-1 rounded-lg hover:scale-95 transition cursor-pointer">
              Sign in
            </button>
          ) : (
...
right-0 z-10 border border-gray-200 w-36 overflow-hidden">

                <div
                  className="flex items-center gap-2 px-3 py-2 cursor-pointer hover:bg-gray-100 transition active:scale-95"
                  onClick={() => navigation("/myorders")}>
                  <img src={FRassets.basket_icon} alt="Orders" className="w-5 h-5" />
                ...
                <div
                  onClick={logout}
                  className="flex items-center gap-2 px-3 py-2 cursor-pointer hover:bg-gray-100 transition active:scale-95">
                  <img src={FRassets.logout_icon} alt="Logout" className="w-5 h-5" />
